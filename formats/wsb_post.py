...
class WSB_Post:
    ...
    def digest_json(self, json_in):
        if 't3' not in json_in['kind']:  # if this isn't a post
            print('Error: not a post handed to digest post', json_in['kind'], file=sys.stderr)
            return

        created_utc = json_in['data']['created_utc']
